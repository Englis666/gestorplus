...
const FormularioAusencia = () => {
  ...
  const validarCampos = () => {
    ...
    if (!fechaInicio)
      erroresTemp.fechaInicio = "La fecha de inicio es obligatoria.";
    if (!fechaFin) erroresTemp.fechaFin = "La fecha de fin es obligatoria.";
    if (fechaInicio && fechaFin && new Date(fechaInicio) > new Date(fechaFin)) {
      erroresTemp.fechaFin =
        "La fecha de fin debe ser posterior a la fecha de inicio.";
    }
    if (!tipoAusencia)
      erroresTemp.tipoAusencia = "Selecciona un tipo de ausencia.";
        ...
        <form onSubmit={handleSolicitarAusencia} noValidate>
          <div className="mb-3">
            ...
            <input
              ...
              className={`form-control ${
                errores.fechaInicio ? "is-invalid" : ""
              }`}
              value={solicitud.fechaInicio}
              onChange={(e) =>
                setSolicitud({ ...solicitud, fechaInicio: e.target.value })
              }
            />
          ...
          <div className="mb-3">
            ...
            <input
              ...
              onChange={(e) =>
                setSolicitud({ ...solicitud, fechaFin: e.target.value })
              }
            />
          ...
          <div className="mb-3">
            <label htmlFor="tipoAusencia" className="form-label">
              Tipo de Ausencia
            </label>
            <select
              id="tipoAusencia"
              className={`form-select ${
                errores.tipoAusencia ? "is-invalid" : ""
              }`}
              value={solicitud.tipoAusencia}
              onChange={(e) =>
                setSolicitud({ ...solicitud, tipoAusencia: e.target.value })
              }
            >
