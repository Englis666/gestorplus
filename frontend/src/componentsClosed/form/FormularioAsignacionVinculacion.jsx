...
const FormularioVinculacion = ({ formData, handleChange, handleSubmit }) => {
    ...
    const campos = [
        { label: "Número de Documento", name: "num_doc", type: "number", icon: "badge" },
    ...
    const validar = (campo = null, valor = null) => {
        ...
        if (!f.fechaFirma)
            nuevosErrores.fechaFirma = "Fecha de firma requerida.";
            ...
            <div className="card-body px-4 animate__animated  animate__faster">
                <form onSubmit={onSubmit} className="animate__animated  animate__faster">
                    ...
                    <div className="d-flex justify-content-end">
                        <button type="submit" className="btn btn-primary px-4 shadow-sm animate__animated animate__fadeInRight animate__delay-1s">
                            <span className="material-icons align-middle me-2">check_circle</span>
                            Asignar vinculación
                        </button>
                    </div>
                </form>
